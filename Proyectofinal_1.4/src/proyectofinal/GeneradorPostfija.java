...
public class GeneradorPostfija {
    ...
    public List<Token> convertir(){
        ...
        for(int i=0; i<infija.size(); i++){
            ...
            if(t.esPalabraReservada()){
            ...
            else if(t.tipo == TipoToken.PARENTESIS_ABIERTO){
                pila.push(t);
            }
            else if(t.tipo == TipoToken.PARENTESIS_CERRADO){
                while(!pila.isEmpty() && pila.peek().tipo != TipoToken.PARENTESIS_ABIERTO){
                    Token temp = pila.pop();
                    postfija.add(temp);
                }
                if(pila.peek().tipo == TipoToken.PARENTESIS_ABIERTO){
                    pila.pop();
                }
                ...
                while(!pila.isEmpty() && pila.peek().tipo != TipoToken.LLAVE_ABIERTA){
                    Token temp = pila.pop();
                    postfija.add(temp);
                }
                postfija.add(t);
            }
            else if(t.tipo == TipoToken.LLAVE_ABIERTA){
                // Se mete a la pila, tal como el parentesis. Este paso
                // pudiera omitirse, sólo hay que tener cuidado en el manejo
                // del "}".
                pila.push(t);
            }
            else if(t.tipo == TipoToken.LLAVE_CERRADA && estructuraDeControl){
